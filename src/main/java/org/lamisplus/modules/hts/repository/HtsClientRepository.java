...
public interface HtsClientRepository extends JpaRepository<HtsClient, Long> {
        ...
        Optional<HtsClient> findTopByPersonUuidAndArchivedAndFacilityId(String personUuid, Integer archived,
                        Long facilityId);
        ...
        String PATIENTS_INFANT_CTE = "WITH pmtct_infants AS (\n" +
        ...
        String PATIENTS_NOT_EXISTS = "AND NOT EXISTS (\n" +
                        "    SELECT 1\n" +
        ...
        @Query(value = PATIENTS_INFANT_CTE + PATIENTS_SELECT_COLS +
                        "WHERE p.archived = ?1 AND p.facility_id = ?2\n" +
                        PATIENTS_NOT_EXISTS +
                        "ORDER BY p.created_date DESC", countQuery = PATIENTS_INFANT_CTE +
                                        "SELECT COUNT(*) FROM patient_person p\n" +
                                        "WHERE p.archived = ?1 AND p.facility_id = ?2\n" +
                                        PATIENTS_NOT_EXISTS, nativeQuery = true)
        ...
        @Query(value = PATIENTS_INFANT_CTE + PATIENTS_SELECT_COLS +
                        ...
                        ")\n" +
                        PATIENTS_NOT_EXISTS, countQuery = PATIENTS_INFANT_CTE +
                                        "SELECT COUNT(*) FROM patient_person p\n" +
                                        "WHERE p.archived = ?1\n" +
                                        "AND (\n" +
                                        "       p.first_name ILIKE ?3\n" +
                                        ...
                                        ")\n" +
                                        PATIENTS_NOT_EXISTS, nativeQuery = true)
        Page<HtsPerson> findAllPersonHtsBySearchParamOptimized(Integer archived, Long facilityId, String search,
                        Pageable pageable);

        String ONLY_PERSON_HTS_CTE = "WITH first_hc AS (\n" +
        ...
        @Query(value = ONLY_PERSON_HTS_CTE + ONLY_PERSON_HTS_SELECT
                        + "ORDER BY fc.id DESC", countQuery = ONLY_PERSON_HTS_CTE +
                                        "SELECT COUNT(*) FROM first_hc fc " +
                                        "INNER JOIN patient_person p ON p.uuid = fc.person_uuid", nativeQuery = true)
        ...
        @Query(value = ONLY_PERSON_HTS_CTE + ONLY_PERSON_HTS_SELECT +
                        ...
                        "ORDER BY fc.id DESC", countQuery = ONLY_PERSON_HTS_CTE +
                                        ...
                                        "INNER JOIN patient_person p ON p.uuid = fc.person_uuid " +
                                        "WHERE p.first_name ILIKE ?3 OR p.surname ILIKE ?3 OR p.other_name ILIKE ?3 " +
                                        "   OR p.hospital_number ILIKE ?3 OR fc.client_code ILIKE ?3", nativeQuery = true)
        Page<HtsPerson> findOnlyPersonHtsBySearchParamOptimized(Integer archived, Long facilityId, String search,
                        Pageable pageable);
        ...
        @Query(value = "WITH hts_patients AS ( \n" +
                        ...
                        "ORDER BY duplicateCount DESC, uniquePersonUuids DESC, firstName, surname, rn", nativeQuery = true)
